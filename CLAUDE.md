...
- `10-mise.sh` — mise 本体は `MISE_VERSION_PIN` との**バージョン比較**で入れ直す、シェル統合は断片ファイルの丸ごと上書き。`mise install` は既存バージョンをスキップするので**あえてスタンプを置かない**（置くと `latest` 指定の `claude` が上流更新に追随できなくなる）。
  - mise 本体のバージョンを固定しているのは暫定処置。2026.8.4 の aqua backend の regression で `claude` が `relative URL without a base` で入らず、readiness probe が 900 秒待って `make up` が落ちる（詳細はスクリプト内のコメント）。上流が直ったらピンを外して latest 追随に戻すこと。**`-x` の存在チェックに戻さないこと**。壊れたバージョンが入っている VM を `make reprovision` で直せなくなる。
